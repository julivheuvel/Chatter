...
class User: 
    ...
    def validate_user(data):
        ...
        if len(data['password']) < 5:
            ...
            is_valid = False
        # print(type(int(data['age'])))
        if int(data['age']) <= 15:
            flash("Must be at least 16 to create an account")
            is_valid = False
    ...
    def get_by_email(cls, data):
        ...
        result = connectToMySQL("chatter").query_db(query, data)
        if len(result) < 1:
            return False    
        return cls(result[0])
    ...
    def get_one(cls, data):
        ...
        results = connectToMySQL('chatter').query_db(query, data)
